...
void detect_collision(Particle* p1, Particle* p2)
{
  ...
  /// GIZEM: add the bind_three_particles part to test it
  if (collision_params.mode & (COLLISION_MODE_BOND | COLLISION_MODE_BIND_THREE_PARTICLES)) {
    int bondG[2];
    ...
    printf("The bond is created between particle %d and %d \n", primary, secondary);
  }
...
void handle_collisions ()
{
  ...
  for (int i = 0; i < number_of_collisions; i++) {
	...
	id2 = collision_queue[i].pp2;
      ...
      /// Gizem: in the tcl script, i used bind_three_particles but why is this error message printed out?
      ERROR_SPRINTF(exceptiontxt, "{exception part --> collision between particles %d and %d} ",
		    id1, id2);
    }
    ...
    /// Gizem: I add this part just to see
    if (collision_params.mode & COLLISION_MODE_BIND_THREE_PARTICLES) {
       printf("three-particle-binding-mode \n");
    }
  }
